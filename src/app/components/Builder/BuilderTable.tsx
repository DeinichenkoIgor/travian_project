...
const BuilderTable: React.FC<{ building: BuildingData }> = ({ building }) => {
  ...
  const generateGridData = () => {
    let constructionTimeReduction;
    if (building.Name === Buildings.MainBuilding.Name) {
        constructionTimeReduction = 100;
    } else {
        constructionTimeReduction = (Buildings.MainBuilding.levels as Record<string, BuildingLevelDetail>)[`lvl${mainBuildingLevel}`]?.ConstructionTime || 100;
    }
